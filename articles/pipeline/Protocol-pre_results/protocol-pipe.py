#!/usr/bin/env python3
"""
Run the full article through protocol/pre-registration evidence review and scoring.
...
_PROTOCOL = Path(__file__).resolve().parent
_PIPELINE = _PROTOCOL.parent
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Full pipeline: spaCy to claims, classify/group, protocol review, score.",
    )
    parser.add_argument(
    ...
    # --- Steps 6-13 (Protocol-pre_results/) ---
    triaged = steps_dir / "triaged.json"
    if start <= 6:
        run_step(
            "Step 6/13 — Triage (protocol)",
            [PY, str(_PROTOCOL / "triage.py"), str(grouped), "-o", str(triaged), "--mappings", str(MAPPINGS)],
            env=base_env,
        )
    ...
    if start <= 7:
        rc_cmd = [
            PY, str(_PROTOCOL / "retrieve_compare.py"), str(triaged),
    ...
    if start <= 8:
        run_step(
            ...
            [PY, str(_PROTOCOL / "prep.py"), str(rc_out), "-o", str(prepped_evidence)],
            env=base_env,
        )
    ...
    if start <= 9:
        run_step(
            ...
            [
                PY, str(_PROTOCOL / "review.py"), str(prepped_evidence),
    ...
    if start <= 10:
        originality_cmd = [
            PY, str(_PROTOCOL / "originality_check.py"),
    ...
    if start <= 13:
        run_step(
            ...
            [
                PY, str(_PROTOCOL / "evidence-doc.py"),
