...
You will be provided with two or more rationales that were each generated from a subset of claims belonging to the same evaluation dimension of a theoretical, review, or narrative paper. Each partial rationale covers a different portion of the evidence but all relate to the same dimension.
...
The partial rationales reference evidence grades (strong, moderate, weak, overclaim, unsupported, unreferenced, unverifiable). When condensing:
- Preserve the overall evidence grade distribution (e.g., "X of Y argumentative claims had strong or moderate citation support")
- Distinguish between claims where cited references actually support the argument vs. claims where the paper's interpretation goes beyond its evidence (overclaim)
- Note the proportion of well-grounded arguments vs. those lacking citation support
- If multiple partials flag the same argumentation gap or overclaim pattern, state it once with emphasis
