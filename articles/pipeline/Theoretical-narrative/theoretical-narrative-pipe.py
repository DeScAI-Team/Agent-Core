#!/usr/bin/env python3
"""
Run the full article through theoretical-narrative evidence review and scoring.

Upstream (claim extraction, same as empirical steps 1-5):
...
Theoretical-narrative folder (steps 6-13):
...
The knowledge base JSONL may live in the same folder or one level up.
Use --kb to point at text_knowledge_base.jsonl explicitly.

All pipeline intermediates live under:
...
_THEORETICAL = Path(__file__).resolve().parent
...
def resolve_run_folder_name(
    ...
    """Paper title from KB or full.md H1 or folder name."""
    doc_name = _first_doc_name_from_kb(kb_path) or input_dir.name
    title = _title_from_fullmd(fullmd_path) or doc_name
    return _safe_research_name(title)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Full pipeline: spaCy to claims, classify/group, theoretical-narrative review, score.",
    )
    parser.add_argument(
        "--input-dir", "--input-pdf-dir",
        type=Path, required=True, metavar="DIR",
        help="Folder with full.md (and optionally grouped.json).",
    )
    parser.add_argument("--kb", type=Path, default=None, metavar="PATH")
    parser.add_argument(
    ...
    print(f"Research folder name: {folder_key}")
    ...
    if start <= 4:
        run_step(
            ...
            [PY, str(_PIPELINE / "classify_claims.py"), "-i", str(validated), "-o", str(classified)],
            env=base_env,
        )
    ...
    if start <= 5:
        run_step(
            ...
            [PY, str(_PIPELINE / "group.py"), str(classified), "-o", str(grouped), "--mappings", str(MAPPINGS)],
            env=base_env,
        )
    ...
    if start <= 6:
        run_step(
            "Step 6/13 — Triage (theoretical-narrative)",
            [PY, str(_THEORETICAL / "triage.py"), str(grouped), "-o", str(triaged), "--mappings", str(MAPPINGS)],
            env=base_env,
        )
    ...
    if start <= 7:
        rc_cmd = [
            PY, str(_THEORETICAL / "retrieve_compare.py"), str(triaged),
            "--kb", str(kb_dest), "--fullmd", str(full_md),
            "--openalex-cache", str(steps_dir / "openalex_cache.json"),
            "-o", str(rc_out),
        ]
        if not use_llm:
    ...
    if start <= 8:
        run_step(
            ...
            [PY, str(_THEORETICAL / "prep.py"), str(rc_out), "-o", str(prepped_evidence)],
            env=base_env,
        )
    ...
    if start <= 9:
        run_step(
            ...
            [
                PY, str(_THEORETICAL / "review.py"), str(prepped_evidence),
                "--mappings", str(MAPPINGS), "-o", str(review_out),
                "--pre-condensed-dump", str(steps_dir / "pre_condensed_rationales.json"),
            ],
            env=base_env,
        )
    ...
    if start <= 10:
        originality_cmd = [
            PY, str(_THEORETICAL / "originality_check.py"),
            "--directory", str(steps_dir), "--fullmd", str(full_md),
            "--kb", str(kb_dest),
            "--openalex-cache", str(steps_dir / "originality_openalex_cache.json"),
            "-o", str(originality_out), "--review", str(review_out),
        ]
        if args.skip_llm:
            originality_cmd.append("--skip-llm")
        run_step(
            f"Step 10/13 — Originality ({'LLM' if use_llm else 'skip-llm'})",
            originality_cmd, env=base_env,
        )

    screener_out = steps_dir / "screener.json"
    if start <= 11:
        screener_cmd = [
            PY, str(_THEORETICAL / "screener.py"),
            "--fullmd", str(full_md),
            "--openalex-cache", str(steps_dir / "openalex_cache.json"),
            "--mappings", str(MAPPINGS), "--review", str(review_out),
            "-o", str(screener_out),
        ]
        if args.skip_llm:
        ...
        run_step(
            ...
            screener_cmd, env=base_env,
        )

    if start <= 12:
        score_cmd = [
            PY, str(_THEORETICAL / "score.py"),
    ...
    if start <= 13:
        run_step(
            ...
            [
                PY, str(_THEORETICAL / "evidence-doc.py"),
