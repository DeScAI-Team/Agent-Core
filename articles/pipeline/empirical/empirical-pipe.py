...
Published artifacts (after scoring and audit export):
  ...
  <output-dir>/<research-folder>/review/review.json   (or legacy .../output/)
  <output-dir>/<research-folder>/review/overview.json
  <output-dir>/<research-folder>/review/evidence_audit.md

With ``--run-dir`` (from run_full_pipeline): fixed ``<run-dir>/steps/`` and ``<run-dir>/review/``.

The folder name uses the paper title from the KB (profile_read_paper heuristic),
not the PDF filename.

Environment: LLM_BASE_URL, LLM_API_KEY (VLLM_* deprecated); --model sets review LLM.
